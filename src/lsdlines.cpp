...
float *line_detector_lsd(unsigned char *image, int w, int h, boundingbox_t bbox, pixel_info_t scale,
                         int *n_out) {
...
int _lsd_line_detector(unsigned char *src, int w, int h,
                       float scaleX, float scaleY, boundingbox_t bbox,
    ...
    for (k = 0; k < n; k++) {
        lines.push_back({lsd_lines[7 * k + 0], lsd_lines[7 * k + 1], lsd_lines[7 * k + 2],
                         lsd_lines[7 * k + 3]});
    }
