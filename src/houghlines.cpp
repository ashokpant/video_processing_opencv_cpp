...
static image_int8u_p
gaussian_sampler_byte_bbox(image_int8u_p in, boundingbox_t bbox, pixel_float_t scale,
                           float sigma_scale) {
...
static void hough_line_standard(unsigned char *image, int w, int h,
    ...
    _mcv_HoughLinesStandard(image, w, h, (float) rho, (float) theta, threshold, INT_MAX, min_theta,
                            max_theta, _lines);
...
_mcv_HoughLinesProbabilistic(int w, int h, unsigned char *_image,
    ...
    for (; count > 0; count--) {
        ...
        if (good_line) {
            line_float_t _lr = {(float) line_end[0].x, (float) line_end[0].y, (float) line_end[1].x,
                                (float) line_end[1].y};
...
static void hough_line_probabilistic(unsigned char *image, int w, int h,
    ...
    _mcv_HoughLinesProbabilistic(w, h, image,
                                 (float) rho, (float) theta, threshold, (int) round(minLineLength),
                                 (int) round(maxGap), INT_MAX, lines);
...
static void _hough_line_detector(unsigned char *src, int w, int h,
                                 float scaleX, float scaleY, float canny_low_thresh,
                                 float canny_high_thresh,
                                 float hough_rho, float hough_theta, float min_theta_linelength,
    ...
    canny_detector(sub_src->data, edge, sub_src->xsize, sub_src->ysize, canny_low_thresh,
                   canny_high_thresh);
...
int HoughLineDetector(unsigned char *src, int w, int h,
                      ...
                      float HoughRho, float HoughTheta, float MinThetaLinelength, float MaxThetaGap,
                      int HoughThresh,
                      HOUGH_LINE_TYPE_CODE _type,
                      boundingbox_t bbox, std::vector<line_float_t> &lines) {
    if (src == NULL)
